...
class PACSDataset(data.Dataset):
    def __init__(self, data_root, domains, train=True, validation=False, transformer=None):
        ...
        self.train = train
        self.val = validation
        full_attributes = np.load(os.path.join(data_root,'photo_resnet18_cumix_imgnet.npy'), allow_pickle=True, encoding='latin1').item()
        self.full_attributes = {}
        for i,c in enumerate(list(full_attributes.keys())):
            self.full_attributes[i] = torch.from_numpy(full_attributes[c]).float()
...
class DomainNetDataset(data.Dataset):
    def __init__(self, data_root, domains, train=True, validation=False, transformer=None):
        ...
        self.train = train
        self.val = validation
        full_attributes = np.load(os.path.join(data_root,'w2v_domainnet.npy'), allow_pickle=True, encoding='latin1').item()
        self.full_attributes = {}
        for i,c in enumerate(list(full_attributes.keys())):
            self.full_attributes[i] = torch.from_numpy(full_attributes[c]).float()
    ...
    def read_single_file(self, filename, id):
        ...
        with open(domain_images_list_path, 'r') as files_list:
            for line in files_list:
                line = line.strip()
                local_path, class_id = line.split(' ')
                self.image_paths.append(os.path.join(self.data_root, local_path))
